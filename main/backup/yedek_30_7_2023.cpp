...
static void do_rt_task(){
    ...
    //volt2 = dmdCur2ADCV2(cur);

    // Analog Output 

    DAC8532_Out_Voltage(channel_A, 3.3); 
    ...
    //if (motor_analog_value >=-0.5 & motor_analog_value <= 0.5)
...
// (1.65* (-1*motor_analog_value + 2.65)/0.4) - 5.4
    ...
    DT = float(end - start)/float(CLOCKS_PER_SEC); // this show the loop time. Look for this, it needs to be locked at 1ms otherwise the control algorithm going to have a problem. 
